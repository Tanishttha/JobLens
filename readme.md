...
Google must allow `https://fake-job-main.onrender.com/auth/google/callback`. The extension can be loaded unpacked from `extension/` in Chrome. The exact selection debounce is owned by `extension/content.js`: every selection clears the previous timer and schedules analysis after exactly 5000 ms.
